...
class CompressionLoss(object):
    ...
    def __init__(self, q=256, lambda_=500, dims=None):
        ...
        self.d_hat = None
        self.d_tilde = None
        self.prior = tf.contrib.distributions.MultivariateNormalFullCovariance(
            loc=np.zeros(dims).astype(np.float32),
            covariance_matrix=np.eye(dims).astype(np.float32))
    ...
    def forward(self, z, sldj, x=None, x_hat=None, x_tilde=None):
        ...
        prior_ll -= np.log(self.q) * self.dims
